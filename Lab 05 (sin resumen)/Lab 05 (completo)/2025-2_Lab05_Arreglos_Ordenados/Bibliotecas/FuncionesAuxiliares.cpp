...
void imprimirReporte(const char *nombre, int *nTotalReproduccionesCanales,double tarifa, //aux y tarifa
    ...
    for (int i = 0; i < cantEtiquetas; i++) {
        ...
        int ratingResumen;
        if (promedioRating>=4) ratingResumen=30;
        else if (promedioRating>=3) ratingResumen=20;
        else if (promedioRating>=2) ratingResumen=0;
        else ratingResumen=-10;
        ...
        if (promedioRating>=4) output << "+30%";
        else if (promedioRating>=3) output << "+20%";
        else if (promedioRating>=2) output << "0%";
        else output << "-10%";
        output << endl;
        output << setw(5) << ' ' << left << setw(40) << "INGRESOS POR REPRODUCCIONES:" << right  << "$ " << setw(13) << tarifa*(tiempoTotal/60)*(tiempoTotal*promedioRating/100) << endl;
        output << endl;
        imprimirLinea(output, '=');
    }
...
void imprimirTitulo(ofstream& output,double tarifa) {
    output << right << setw((anchoDoc/2)+10) << "PLATAFORMA TP_Twitch" << endl;
