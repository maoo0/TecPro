...
void imprimirReporte(const char *nombre, int *nTotalReproduccionesCanales,double tarifa, //aux y tarifa
    ...
    int posCanal, k=0, totalReproResumen=0,timepoTotalRepros=0, tiempoTotal, ingresosPorReproResumen;  ;
    double ratingAcumulado, promedioRating;
        ...
        for (int j = 0; j < cantExC; j++) {
            if (codeEtiquetas[i]==codeEtiquetasExC[j]) {
                ...
                if (posCanal!=-1) {
                    ...
                    output << left << setw(30)<< ratingCanales[posCanal];
                    output << left << setw(35)<< nTotalReproduccionesCanales[posCanal];
                    tiempoTotal = nTotalReproduccionesCanales[posCanal]*duracionEtiquetas[i];
                    imprimirTiempo(output, tiempoTotal);
                    output << endl;
                    k++;
                    totalReproResumen+=nTotalReproduccionesCanales[posCanal];
                    timepoTotalRepros+=tiempoTotal;
                    ratingAcumulado+=ratingCanales[posCanal];
                }
                else cout << "q fo " << i << ' ' << j<< ' ' << k << endl;
            }
        }
        //resumen (me dio flojera hacerlo funcion)
        imprimirLinea(output, '-');
        promedioRating= ratingAcumulado/ k;
        double ratingResumen;
        output << setw(3) << ' ' <<    "RESUMEN DE LA ETIQUETA:" << endl;
        output << setw(5) << ' ' << left << setw(40) << "TOTAL DE REPRODUCCIONES:" << right << setw(15) <<totalReproResumen << endl;
        output << setw(5) << ' ' << left << setw(47) << "TIEMPO TOTAL REPRODUCIDO:" ;
        imprimirTiempo(output, timepoTotalRepros);
        output << endl;

        output << setw(5) << ' ' << left << setw(40) << "PORCENTAJE PROMEDIO POR RATING:" << right << setw(15);
        if (promedioRating>=4) output << "+30%", ratingResumen=1.30;
        else if (promedioRating>=3) output << "+20%", ratingResumen=1.20;
        else if (promedioRating>=2) output << "0%", ratingResumen=1;
        else output << "-25%", ratingResumen=0.75;
        output << endl;

        output << setw(5) << ' ' << left << setw(40) << "INGRESOS POR REPRODUCCIONES:" << right  << setw(9) <<"$ " << setw(6) << tarifa*(tiempoTotal/60)*ratingResumen << endl;
        output << endl;
        imprimirLinea(output, '=');
    }
...
void imprimirTitulo(ofstream& output,double tarifa) {
    output << right << setw((anchoDoc+10)/2) << "PLATAFORMA TP_Twitch" << endl;
    output << setw(60) << "TARIFA POR DURACION DE LAS ETIQUETAS: " << tarifa << " POR CADA MINUTO Y FRACCION" << endl;
    imprimirLinea(output, '=');
}
...
int leerFecha(ifstream &input) { //podria ser saltar fecha
